...
P3D GeneralizedCoordinatesRobotRepresentation::
    ...
    V3D vLocal(joint ? joint->cJPos : P3D(), pLocal); // if there is no joint use (0,0,0) to handle the world frame rotation
    V3D vParent = rotateVec(vLocal, q[qIndex], getQAxis(qIndex));
    P3D pParent = (joint ? joint->pJPos : P3D()) + vParent; 
...
P3D GeneralizedCoordinatesRobotRepresentation::getWorldCoordinates(const P3D &p,
    ...
    return P3D(q[0], q[1], q[2]) + pInWorld;
}
...
void GeneralizedCoordinatesRobotRepresentation::compute_dpdq(const P3D &p,
    ...
    P3D pLocal = p;
    for (int qIndex = getQIdxForJoint(rb->pJoint); qIndex > 2; qIndex = getParentQIdxOf(qIndex))
    {
        RBJoint *joint = getJointForQIdx(qIndex);
        V3D vWorld =  joint ? 
             V3D(getWorldCoordinates(joint->cJPos, joint->child), getWorldCoordinates(p, rb)) : 
             getWorldRotationForQ(qIndex) * V3D(pLocal);
        
        V3D dpdq_i = getWorldCoordsAxisForQ(qIndex).cross(vWorld);
        dpdq(0, qIndex) = dpdq_i[0];
        dpdq(1, qIndex) = dpdq_i[1];
        dpdq(2, qIndex) = dpdq_i[2];
